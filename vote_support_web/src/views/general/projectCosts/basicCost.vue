<template>
  <div class="app-container">
    <!--添加新科目对话框表单-->
    <div class="cost-dialog"><el-dialog title="添加新报销项目" :visible.sync="addFormVisible" style="width: 100%">
      <el-form :rules="rules" :model="ruleForm" ref="ruleForm" label-width="100px" size="mini">
        ...
        <el-form-item label="工程名称" prop="pro_id">
          <el-select
        ...
        <el-form-item label="备注" prop="note">
          <el-input type="textarea" autosize style="width: 300px" v-model="ruleForm.note" auto-complete="off"></el-input>
        </el-form-item>
      </el-form>
    ...
    <div class="searchForm-style-01">
      <el-form :inline="true" ref="searchForm" :model="searchForm" label-width="100px">
        ...
        <el-row class="rowStyle-01">
          ...
          <el-form-item>
            <el-input @keyup.enter.native="onQuery" size="small" style="width: 250px" v-model="searchForm.pro_name" placeholder="项目名称,输入关键字即可"></el-input>
          </el-form-item>
          <el-form-item>
            <el-select @keyup.enter.native="onQuery" size="small" v-model="searchForm.cost_user_id" filterable remote reserve-keyword placeholder="报销人" :remote-method="remoteCostUsers">
              <el-option v-for="costUser in costUsers" :key="costUser.id" :label="costUser.name" :value="costUser.id"></el-option>
            </el-select>
          </el-form-item>
          <el-form-item>
            <el-select @keyup.enter.native="onQuery" size="small" v-model="searchForm.branch_id" placeholder="选择分公司">
              <el-option v-for="b in branch" :key="b.id" :label="b.name" :value="parseInt(b.id)"></el-option>
            </el-select>
          </el-form-item>
          <el-form-item>
        ...
        <el-row class="rowStyle-01">
          <div class="actionBar">
            <el-form-item label="操作选项">
              <!--添加新项-->
              <el-button size="small" @click="onAdd" type="danger" icon="el-icon-edit">添加报销记录</el-button>
              <!--导入EXCEL-->
        ...
        <el-row class="rowStyle-01">
          <el-form-item label="表项显示">
            ...
            <el-checkbox v-model="show_project_number">项目编号</el-checkbox>
            <el-checkbox v-model="show_project_name">项目名称</el-checkbox>
            ...
            <el-checkbox v-model="show_valid" >有效性</el-checkbox>
    ...
    <!--基础报销表格-->
    <div>
      <el-table border :data="costs" id="cost" :row-style="rowStyle" :cell-style="cellStyle" stripe fit show-summary>
        ...
        <el-table-column :show-overflow-tooltip="true" align="center" prop="proof_no" label="凭证号" width="120" v-if="show_proof"></el-table-column>
        <el-table-column align="center" prop="pro_number" label="项目编号" width="300" v-if="show_project_number"></el-table-column>
        <el-table-column :show-overflow-tooltip="true" align="center" prop="project.name" label="工程名称" v-if="show_project_name" width="300"></el-table-column>
        ...
        <el-table-column align="center" prop="input_user_name" label="录入人" width="120" v-if="show_input_user"></el-table-column>
        ...
        <el-table-column align="right" header-align="center" prop="money" label="金额" width="100"></el-table-column>
        <el-table-column :show-overflow-tooltip="true" align="center" prop="use" label="用途" width="280" v-if="show_use"></el-table-column>
        <el-table-column :show-overflow-tooltip="true" align="center" prop="note" label="备注" width="150" v-if="show_note"></el-table-column>
        ...
        <el-table-column align="center" label="操作" width="150" v-if="show_action">
          <template slot-scope="scope">
            <el-button @click="onUpdate(scope.row.id)" size="mini" type="warning" class="btnUpdate" icon="el-icon-edit">修改</el-button>
...
<!--
    ...
    页面访问:     read_basicCost               beforeMount()
    添加记录:     create_costBasic             onAdd()
    修改记录:     update_costBasic             onUpdate()
    Excel导入:    create_costBasic_Excel      onAddExcel()
    显示用途和备注:read_basicCost_UseAndNote  checkUseAndNote()
    -
    跳转报销科目:  read_sbjType                 onGoToSbj()
    -->
<script>
  import { getBasic, getType, getSbjType, createBasic } from '../../../api/costBasic'
  import { parseTime } from '../../../utils/index'
  import { getBranches } from '../../../api/branch'
  import { getUsers } from '../../../api/user'
  import { getProjects } from '../../../api/project'
  import { fixed } from '../../../api/costApply'
  import uploadExcel from '../uploadExcelBasic.vue'
  ...
  export default {
    ...
    data() {
      return {
        ...
        rules: {
          date: [
            { required: true, message: '请选择面单日期', trigger: 'blur' }
          ],
          cost_user_id: [
    ...
    methods: {
      ...
      onAdd() {
        /** 权限检查 */
        if (this.actionCheck_.actionCheck('create_costBasic#') === false) {
      ...
      onAddExcel() {
        /** 权限检查 */
        if (this.actionCheck_.actionCheck('create_costBasic_Excel#') === false) {
          this.$message.error('错误提示,您没有该操作的权限!')
          return false
        }
      ...
      onUpdate(id) {
        /** 权限检查 */
        if (this.actionCheck_.actionCheck('update_costBasic#') === false) {
          this.$message.error('错误提示,您没有该操作的权限!')
          return false
        }
        this.activeID = id
      ...
      submitForm(formName) {
        this.$refs[formName].validate((valid) => {
          if (valid) {
            ...
            createBasic(this.ruleForm).then(res => {
    ...
    beforeMount() {
      /**  页面权限检查 如无权限则跳转至404  */
      if (this.actionCheck_.actionCheck('read_basicCost#') === false) {
