<template>
  <div class="app-container">
    ...
    <!--</table>-->
    <div id="edit">
      <el-dialog title="修改项目分包属性" :visible.sync="dialogEdit" width="30%">
    ...
    <div id="history">
      <el-dialog title="工人结算记录" :visible.sync="dialog" width="50%">
        <table class="_table" border="0" cellpadding="0" cellspacing="0" style="width: 100%">
          <thead>
          <tr>
            <th>序号</th>
...
<!--
    ...
    页面访问: read_laoWu_FH             beforeMount()
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      onNewDialog(item) {
        /** 权限检查 */
        if (this.actionCheck_.actionCheck('read_laoWuTotal_FH#') === false) {
      ...
      onSubmitAdd() {
        /** 权限检查 */
        if (this.actionCheck_.actionCheck('create_laoWu_FH#') === false) {
          this.$message.error('错误提示,您没有该操作的权限!')
          return false
        }
      ...
      onSubmitEdit(item) {
        /** 权限检查 */
        if (this.actionCheck_.actionCheck('update_fen_type_FH#') === false) {
          this.$message.error('错误提示,您没有该操作的权限!')
          return false
        }
      ...
      onSubmitStatus(item) {
        /** 权限检查 */
        if (this.actionCheck_.actionCheck('update_fen_status_FH#') === false) {
          this.$message.error('错误提示,您没有该操作的权限!')
          return false
        }
      ...
      remoteMethod(query) {
        ...
        params = { branch_id: 3 }
        if (query.trim() !== '') {
          params = { name: query.trim(), branch_id: 3 }
        }
      ...
      onSubmitFenAdd(item) {
        /** 权限检查 */
        if (this.actionCheck_.actionCheck('create_laoWuTeam_FH#') === false) {
          this.$message.error('错误提示,您没有该操作的权限!')
          return false
        }
      ...
      onSubmitFenView(item, id) {
        // console.log(id)
        /** 权限检查 */
        if (this.actionCheck_.actionCheck('read_laoWuTeam_FH#') === false) {
          this.$message.error('错误提示,您没有该操作的权限!')
          return false
        }
        this.dialog = true
      ...
      submitHisEdit(item) {
        /** 权限检查 */
        if (this.actionCheck_.actionCheck('update_laoWuTeam_FH#') === false) {
          this.$message.error('错误提示,您没有该操作的权限!')
          return false
        }
        this.isAdd = true
    ...
    beforeMount() {
      /**  页面权限检查 如无权限则跳转至404  */
      if (this.actionCheck_.actionCheck('read_laoWu_FH#') === false) {
