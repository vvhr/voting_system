...
<script>
  ...
  import { createBasicExcel, checkBasicExcel } from '../../api/costBasic'
  ...
  export default {
    ...
    methods: {
      ...
      handleSuccess({ results, header }) {
        ...
        if (header.length !== 11) {
          this.$message({
            message: '导入错误，表头长度非法，请确认表头是否是8个属性！',
            type: 'warning'
          })
          this.errorNote = this.errorNote + '<br /> 表头长度非法，请确认表头是否是8项属性。'
          this.excelCheck = '检查不通过'
          return false
        }
        if (this.checkHeader(0, '面单日期') === false) return false
        if (this.checkHeader(1, '凭证号') === false) return false
        ...
        for (let i = 0; i < this.tableData.length; i++) {
          // 1.判断日期是否合法，转字符串后取日期长度,若不合法则跳到下一条
      ...
      //   createBasicExcel(this.ruleForm).then(res => {
      ...
      //       this.$parent.completeBtnVisiable = true
      //       this.resetForm()
      //       return true
      //     } else {
      //       if (res.data.bCode === 102) {
      //         this.errorData = this.errorData + '<br />' + res.data.bError
      //         console.log(res.data.bError)
      //         this.$notify({
      //           title: '出错',
      //           message: '新增失败',
      //           type: 'warning',
      ...
      check(i, length) {
        ...
        this.ruleForm.date = Math.floor(this.ruleForm.date / 1000)
        this.ruleForm.proof_no = this.tableData[i].凭证号
        this.ruleForm.pro_name = this.getTrim(this.tableData[i].分组名称)
        ...
        this.sleep(this.addSpeed).then(() => {
          // 执行提交
          checkBasicExcel(this.ruleForm).then(res => {
