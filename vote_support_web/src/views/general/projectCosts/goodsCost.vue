<template>
  <div class="app-container">
    ...
    <!--基础报销表格-->
    <div>
      <el-table border :data="costs" id="goods" :row-style="rowStyle" :cell-style="cellStyle" stripe fit show-summary>
...
<!--
    ...
    页面访问:     read_goodsCost               beforeMount()
    添加记录:     create_costGoods             onAdd()
    修改记录:     update_costGoods             onUpdate()
    Excel导入:    create_costGoods_Excel      onAddExcel()
    *****待完成***** 显示用途和备注:read_goodsCost_UseAndNote  checkUseAndNote()
    -
    跳转报销科目:  read_sbjType                 onGoToSbj()
    -->
<script>
  import { getGoods, getType, createGoods } from '../../../api/costGoods'
  import { getSbjType } from '../../../api/costGeneral'
  import { parseTime } from '../../../utils/index'
  import { getBranches } from '../../../api/branch'
  import { getUsers } from '../../../api/user'
  import { getProjects } from '../../../api/project'
  import { fixed } from '../../../api/costApply'
  import uploadExcel from '../uploadExcelGoods.vue'
  ...
  export default {
    ...
    methods: {
      ...
      onAdd() {
        /** 权限检查 */
        if (this.actionCheck_.actionCheck('create_costGoods#') === false) {
      ...
      onAddExcel() {
        /** 权限检查 */
        if (this.actionCheck_.actionCheck('create_costGoods_Excel#') === false) {
          this.$message.error('错误提示,您没有该操作的权限!')
          return false
        }
      ...
      onUpdate(id) {
        /** 权限检查 */
        if (this.actionCheck_.actionCheck('update_costGoods#') === false) {
          this.$message.error('错误提示,您没有该操作的权限!')
          return false
        }
        this.activeID = id
    ...
    beforeMount() {
      /**  页面权限检查 如无权限则跳转至404  */
      if (this.actionCheck_.actionCheck('read_goodsCost#') === false) {
