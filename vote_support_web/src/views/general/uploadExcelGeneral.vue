...
<script>
  ...
  import { createGeneralExcel, checkGeneralExcel } from '../../api/costGeneral'
  ...
  export default {
    ...
    methods: {
      ...
      handleSuccess({ results, header }) {
        ...
        if (header.length !== 11) {
          this.$message({
            message: '导入错误，表头长度非法，请确认表头是否是11项！',
            type: 'warning'
          })
      ...
      //   return createGeneralExcel(this.ruleForm).then(res => {
      ...
      //         })
      //       }
      //       this.errorData = this.errorData + '<br />★ 第' + (i + 1) + '条数据添加失败，原因：' + res.data.bError
      ...
      check(i, length) {
        ...
        this.ruleForm.cost_branch_name = this.getTrim(this.tableData[i].分公司)
        ...
        this.sleep(150).then(() => {
          // 执行提交
          checkGeneralExcel(this.ruleForm).then(res => {
      ...
      create(i, length) {
        ...
        this.sleep(200).then(() => {
        // 执行提交
          createGeneralExcel(this.ruleForm).then(res => {
            // console.log(this.ruleForm)
            if (res.data.bCode === 101) {
              this.$notify({
                title: '综合报销数据添加成功' + 'x' + (i + 1),
                type: 'success',
                position: 'top-right'
              })
              this.successNumber = this.successNumber + 1
